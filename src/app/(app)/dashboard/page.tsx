...
function UserDashboard() {
  ...
  // Error 1: Potential issue with undefined session state
  const baseUrl = `${window.location.protocol}//${window.location.host}`;
  const profileUrl = session && session.user ? `${baseUrl}/u/${username}` : "";

  // Error 2: Handle empty or undefined state for acceptMessages (Switch)
